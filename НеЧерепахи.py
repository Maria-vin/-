...
flykm = Entry(width=10, state=DISABLED)
...
trainkm = Entry(width=10, state=DISABLED)
...
gruzautokm = Entry(width=10, state=DISABLED)
...
gazelkm = Entry(width=10, state=DISABLED)
...
def Sum(*args):
    if float(Weight.get()) <= 10000 and float(Length.get()) <= 100 and float(Width.get()):
        if check1.get() == 1:
            fl = float(flykm.get())
            if check2.get() == 1:
